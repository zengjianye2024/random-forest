###Data Preprocessing
gphylum <- read.csv('6species.csv',header=T,row.names = 1)
##The number 6 in 6species represents the muscle.
gphylum <- gphylum[which(rowSums(gphylum) >= 0.0005), ]#Filter low abundance microorganisms
gsj <- read.csv('6msj.csv',header=T,row.names = 1)#For 6msj, '6' represents the muscle sample, 'm' is the first letter of muscle, and 'sj' represents PMI.
...
form_reg <- as.formula(paste0("PMI~",paste(colnames(traindata)[1:594],collapse="+")))
form_reg
set.seed(123)
...
importance_otu.select <- importance_otu[1:78, ]#Determined based on the cross validation curve and the values of Rsquared and MAE
...
library(openxlsx)
write.xlsx(importance_otu.select, file = "Muscle species importance_table.xlsx", rowNames = FALSE)
