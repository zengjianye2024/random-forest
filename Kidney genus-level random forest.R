###Data Preprocessing
gphylum <- read.csv('5genus.csv',header=T,row.names = 1)
##The number 5 in 5genus represents the spleen.
...
form_reg <- as.formula(paste0("PMI~",paste(colnames(traindata)[1:83],collapse="+")))
form_reg
set.seed(123)
...
library(openxlsx)
write.xlsx(importance_otu.select, file = "Kidney genus importance_table.xlsx", rowNames = FALSE)
