###Data Preprocessing
gphylum <- read.csv('3species.csv',header=T,row.names = 1)
##The number 3 in 3species represents the spleen.
...
form_reg <- as.formula(paste0("PMI~",paste(colnames(traindata)[1:179],collapse="+")))
form_reg
set.seed(123)
...
importance_otu.select <- importance_otu[1:7, ]#Determined based on the cross validation curve and the values of Rsquared and MAE
...
library(openxlsx)
write.xlsx(importance_otu.select, file = "Spleen species importance_table.xlsx", rowNames = FALSE)
