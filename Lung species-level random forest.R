###Data Preprocessing
gphylum <- read.csv('4species.csv',header=T,row.names = 1)
##The number 4 in 4species represents the lung.
gphylum <- gphylum[which(rowSums(gphylum) >= 0.0005), ]#Filter low abundance microorganisms
gsj <- read.csv('4lsj.csv',header=T,row.names = 1)#For 4lsj, '4' represents the lung sample, 'l' is the first letter of lung, and 'sj' represents PMI.
...
form_reg <- as.formula(paste0("PMI~",paste(colnames(traindata)[1:420],collapse="+")))
form_reg
set.seed(123)
...
importance_otu.select <- importance_otu[1:25, ]#Determined based on the cross validation curve and the values of Rsquared and MAE
...
library(openxlsx)
write.xlsx(importance_otu.select, file = "Lung species importance_table.xlsx", rowNames = FALSE)
