###Data Preprocessing
gphylum <- read.csv('7species.csv',header=T,row.names = 1)
##The number 7 in 7species represents the gut.
gphylum <- gphylum[which(rowSums(gphylum) >= 0.0005), ]#Filter low abundance microorganisms
gsj <- read.csv('7gsj.csv',header=T,row.names = 1)#For 7gsj, '7' represents the gut sample, 'g' is the first letter of gut, and 'sj' represents PMI.
...
form_reg <- as.formula(paste0("PMI~",paste(colnames(traindata)[1:407],collapse="+")))
form_reg
set.seed(123)
...
importance_otu.select <- importance_otu[1:80, ]#Determined based on the cross validation curve and the values of Rsquared and MAE
...
library(openxlsx)
write.xlsx(importance_otu.select, file = "Gut species importance_table.xlsx", rowNames = FALSE)
