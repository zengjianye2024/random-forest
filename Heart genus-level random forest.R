###Data Preprocessing
gphylum <- read.csv('1genus.csv',header=T,row.names = 1)
##The number 1 in 1genus represents the heart.
...
form_reg <- as.formula(paste0("PMI~",paste(colnames(traindata)[1:127],collapse="+")))
form_reg
set.seed(123)
...
importance_otu.select <- importance_otu[1:38, ]#Determined based on the cross validation curve and the values of Rsquared and MAE
...
library(openxlsx)
write.xlsx(importance_otu.select, file = "heart genus importance_table.xlsx", rowNames = FALSE)
