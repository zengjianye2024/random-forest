###Data Preprocessing
gphylum <- read.csv('1species.csv',header=T,row.names = 1)
##The number 1 in 1species represents the heart, 2 represents the liver, 3 represents the spleen, 4 represents the lungs, 5 represents the kidneys, 6 represents the muscles, and 7 represents the intestines.
gphylum <- gphylum[which(rowSums(gphylum) >= 0.0005), ]#Filter low abundance microorganisms
gsj <- read.csv('1hsj.csv',header=T,row.names = 1)#For 1hsj, '1' represents the heart sample, 'h' is the first letter of heart, and 'sj' represents PMI.
...
form_reg <- as.formula(paste0("PMI~",paste(colnames(traindata)[1:1055],collapse="+")))
form_reg
set.seed(123)
...
importance_otu.select <- importance_otu[1:139, ]#Determined based on the cross validation curve and the values of Rsquared and MAE
...
library(openxlsx)
write.xlsx(importance_otu.select, file = "heart species importance_table.xlsx", rowNames = FALSE)
