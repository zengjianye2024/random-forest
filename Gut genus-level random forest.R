###Data Preprocessing
gphylum <- read.csv('7genus.csv',header=T,row.names = 1)
##The number 7 in 7genus represents the gut.
...
form_reg <- as.formula(paste0("PMI~",paste(colnames(traindata)[1:144],collapse="+")))
form_reg
set.seed(123)
...
importance_otu.select <- importance_otu[1:19, ]#Determined based on the cross validation curve and the values of Rsquared and MAE
...
library(openxlsx)
write.xlsx(importance_otu.select, file = "Gut genus importance_table.xlsx", rowNames = FALSE)
