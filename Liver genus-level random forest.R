###Data Preprocessing
gphylum <- read.csv('2genus.csv',header=T,row.names = 1)
##The number 2 in 2genus represents the liver.
...
form_reg <- as.formula(paste0("PMI~",paste(colnames(traindata)[1:91],collapse="+")))
form_reg
set.seed(123)
...
importance_otu.select <- importance_otu[1:40, ]#Determined based on the cross validation curve and the values of Rsquared and MAE
...
library(openxlsx)
write.xlsx(importance_otu.select, file = "liver genus importance_table.xlsx", rowNames = FALSE)
