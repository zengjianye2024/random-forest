###Data Preprocessing
gphylum <- read.csv('6genus.csv',header=T,row.names = 1)
##The number 6 in 6genus represents the muscle.
...
form_reg <- as.formula(paste0("PMI~",paste(colnames(traindata)[1:198],collapse="+")))
form_reg
set.seed(123)
...
importance_otu.select <- importance_otu[1:12, ]#Determined based on the cross validation curve and the values of Rsquared and MAE
...
library(openxlsx)
write.xlsx(importance_otu.select, file = "Muscle genus importance_table.xlsx", rowNames = FALSE)
