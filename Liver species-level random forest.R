###Data Preprocessing
gphylum <- read.csv('2species.csv',header=T,row.names = 1)
##The number 2 in 2species represents the liver.
gphylum <- gphylum[which(rowSums(gphylum) >= 0.0005), ]#Filter low abundance microorganisms
gsj <- read.csv('2lsj.csv',header=T,row.names = 1)#For 2lsj, '2' represents the liver sample, 'l' is the first letter of liver, and 'sj' represents PMI.
...
form_reg <- as.formula(paste0("PMI~",paste(colnames(traindata)[1:210],collapse="+")))
form_reg
set.seed(123)
...
importance_otu.select <- importance_otu[1:18, ]#Determined based on the cross validation curve and the values of Rsquared and MAE
...
library(openxlsx)
write.xlsx(importance_otu.select, file = "liver species importance_table.xlsx", rowNames = FALSE)
