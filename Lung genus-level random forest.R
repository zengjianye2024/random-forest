###Data Preprocessing
gphylum <- read.csv('4genus.csv',header=T,row.names = 1)
##The number 4 in 4genus represents the lung.
...
form_reg <- as.formula(paste0("PMI~",paste(colnames(traindata)[1:129],collapse="+")))
form_reg
set.seed(123)
...
library(openxlsx)
write.xlsx(importance_otu.select, file = "Lung genus importance_table.xlsx", rowNames = FALSE)
